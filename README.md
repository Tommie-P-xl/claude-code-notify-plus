...
> Claude Code 在电脑上写代码，你在手机上审批决策。终端、微信、QQ 三端联动，随时随地掌控 AI 工作流。

当 Claude Code 需要你的输入时 — 无论是审批文件写入、选择技术方案，还是回答多选问题 — 你都可以直接在微信或 QQ 上回复，无需回到电脑前。

---

## 功能亮点

- **三端回复** — 终端、微信、QQ 任一渠道回复即可，先到先生效
- **审批遥控** — 远程批准/拒绝文件操作、命令执行，支持"本次通过"和"全部通过"
- **选择题遥控** — 远程回答 Claude Code 的单选、多选问题
...
hook 触发 → 解析上下文 → 提取选项 → 创建 pending 文件
                    ...
                    ┌───────────────────┼───────────────────┐
                    ▼                   ▼                   ▼
               终端显示选项         微信发送通知         QQ 发送通知
               等待键盘输入        等待用户回复         等待用户回复
                    │                   │                   │
                    └───────────────────┼───────────────────┘
                                        ▼
                              responses/{id}.json
...
| 渠道 | 稳定性 | 说明 |
|------|--------|------|
| **QQ** | **推荐** | 通过 QQ Bot API 发送，配置 AppID/AppSecret 即可 |
| **Windows Toast** | 稳定 | 系统原生弹窗通知，带提示音 |
| **微信** | 容易过期 | 通过 ilink Bot API 发送，`context_token` 会过期需手动恢复 |

> **推荐 QQ 作为主要远程通知渠道。** 微信的 `context_token` 会随时间过期（通常数小时到一天），过期后需在微信中给 bot 发消息恢复。QQ 没有此问题。
...
**Q: 微信收不到通知？**
- 可能是 `context_token` 过期，在 Web UI 重新扫码登录

**Q: 标签用完了怎么办？**
- 超过 5 分钟未响应的请求会自动清理，标签从 A 重新开始
