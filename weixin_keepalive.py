...
# 微信 iLink API 常量（与 Hermes Agent 保持一致）
    ...
    while True:
        ...
        if not enabled or not token:
            ...
            if startup_retries <= 5:
                log(f"微信配置未就绪，等待重试 ({startup_retries}/5)...")
                time.sleep(2)
                continue
            ...
            if ret == 0 and errcode == 0:
                ...
                if msgs:
                    for msg in msgs:
                        ...
                        # 自动获取发送者的 user_id 作为 to_user_id（与 QQ/Telegram 行为一致）
                        from_user = msg.get("from_user_id", "")
                        if from_user:
                            cfg = load_config()
                            if cfg.get("weixin", {}).get("to_user_id") != from_user:
                                cfg["weixin"]["to_user_id"] = from_user
                                save_config(cfg)
                                log(f"[weixin] 获取到 to_user_id: {from_user}")
                ...
                # bot session 过期，无法恢复，需要用户重新扫码登录
                log("[weixin] bot session 过期 (errcode=-14)，需要重新扫码登录，保活退出")
                break
            else:
